...
def print_metrics(y_true, y_pred, y_score, eval_results, level, logger):
    ...
    logger.info(f"ACC: {accuracy:.4f}; P: {precision:.4f}; R: {recall:.4f}; F1: {f1:.4f}; ROC-AUC: {roc_auc:.4f}; AP: {ap:.4f}")
...
class MedCocoMetric(BaseMetric):
    """Medical COCO evaluation metric.

    Evaluates metrics for series and slice levels.
    ...
    """
    default_prefix: Optional[str] = 'med'
    ...
    def results2json(self, results: Sequence[dict],
        ...
        for idx, result in enumerate(results):
            ...
            for i, label in enumerate(labels):
                ...
                print('results2json: label', label)
